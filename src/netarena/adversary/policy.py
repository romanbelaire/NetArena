"""Tabular SARSA adversary for Route curriculum."""
...
class TabularSarsa:
    ...
    def select_action(
        ...
        *,
        ...
        # Soft coverage: probe unvisited only when we have no exploit target yet,
        # or when remaining budget is too small to cover leftovers later.
        if (
            not greedy
            and unvisited
            and n_unvisited > 0
    ...
    def update(
        ...
        *,
        ...
        if next_action < 0 or next_action >= self.n_arms:
            raise ValueError(f"next_action out of range: {next_action}")

        q_sa = self._ensure(state)
        if done:
            target = reward
        else:
            q_ns = self._ensure(next_state)
            target = reward + self.gamma * q_ns[next_action]
    ...
    @classmethod
    def load(cls, path: str | Path, seed: int | None = None) -> "TabularSarsa":
